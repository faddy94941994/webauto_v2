...
if (isset($_POST['show100'])) {
	$sql_select = "SELECT * FROM change_spinner ORDER BY id DESC LIMIT 100";
}elseif (isset($_POST['show500'])) {
	$sql_select = "SELECT * FROM change_spinner ORDER BY id DESC LIMIT 500";
}elseif (isset($_POST['show1000'])) {
	$sql_select = "SELECT * FROM change_spinner ORDER BY id DESC LIMIT 1000";
}elseif (isset($_POST['showAll'])) {
	$sql_select = "SELECT * FROM change_spinner ORDER BY id DESC";
}elseif (isset($_POST['search'])) {
	$phone = $_POST['phone_dp'];
	$vowels = array($load_db_setting->agent);
	$str = $phone;
	$test = str_replace($vowels, "", $str);
	$sql_select = "SELECT * FROM change_spinner WHERE phone LIKE '%$phone%' OR name LIKE '%$test%' OR username LIKE '%$test%' ORDER BY id DESC";	
}else{
...
$sql_select = "SELECT * FROM change_spinner WHERE date_change BETWEEN '$FromDay' AND '$ToDay' ORDER BY id DESC";	
