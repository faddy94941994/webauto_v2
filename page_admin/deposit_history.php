	...
	<span class="logo-mini"><img src="<?php echo $logo; ?>" width="150" alt=""></span> <span class="logo"></a></h3>
		...
		<ul class="sidebar-menu" data-widget="tree">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <a href="/admin/deposit_history" class="dropdown-item active"><i class="fas fa-history me-2"></i>ประวัติ รายการฝาก </a>
...
<div class="bg-secondary rounded p-4 m-10" style= margin:30px;>
		...
		<div class="row">
			<div class="col-md-6">
				<div class="info-box">
					<h4 class="text-black"><i class="fas fa-history"></i> เช็คข้อมูลการฝาก</h4>
					<hr>
					<div class="row mb-3">
						<div class="col-md-8">
...
function showdatelist(GetDate) {
	...
	$.each(GetDate.result, function( index, value ) {
		var str_confirm_dp;
		if(value.confirm_dp == 'รอดำเนินการ'){
			str_confirm_dp = '<span class="btn btn-info btn-sm noHover"><i class="fas fa-spinner fa-spin"></i> กำลังดำเนินการ</span>';
		}
		if(value.confirm_dp == 'อนุมัติ'){
			str_confirm_dp = '<span class="btn btn-sm btn-success px-2 noHover"><i class="fas fa-check"></i> อนุมัติ</span>';
		}
		if(value.confirm_dp == 'ปฏิเสธ'){
			str_confirm_dp = '<span class="btn btn-sm btn-danger px-2 noHover"><i class="fas fa-times"></i> ปฏิเสธ</span>';
		}
		
		if(value.confirm_dp == 'หมดเวลา'){
			str_confirm_dp = '<span class="btn btn-sm btn-danger px-2 noHover"><i class="fas fa-alarm-exclamation"></i> หมดเวลา</span>';
		}
		
		html += '<tr>';
		html += '<th scope="col">' + str_confirm_dp + '</th>';
		html += '<th scope="col"><?php echo $LoadConfig->user_agent; ?>' + value.username_dp + '</th>';
		html += '<th scope="col">' + value.amount_dp + '</th>';
		html += '<th scope="col">' + value.creditbefore + '</th>';
		html += '<th class="text-decoration-none">' + value.bankin_dp + '</th>';
		html += '<th scope="col">' + value.promotion_dp + '</th>';
		html += '<th scope="col">' + value.phone_dp + '</th>';
		html += '<th scope="col">' + value.name_dp + '</th>';
		html += '<th scope="col">' + value.add_dp + '</th>';
		html += '<th scope="col">' + value.edit_dp + '</th>';
		html += '<th scope="col" class="text-nowrap">' + value.date_dp + '</th>';
		html += '<th scope="col"><a href="/admin/depositupdateform?id=' + value.id + '" class="btn btn-sm btn-info px-2"><i class="fas fa-edit"></i> แก้ไข</a></th>';
		html += '</tr>';
	});
...
$.ajax({
      type: "POST",
      url: "/api/admin/loaddeposithistory",
      data: {
          search:search,
